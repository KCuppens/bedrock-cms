...
class SessionFactory(BaseFactory):
    ...
    ended_at = factory.LazyAttribute(lambda obj: 
        fake.date_time_between(start_date=obj.started_at, end_date='+1h')
    )
...
class EventFactory(BaseFactory):
    ...
    event_type = factory.Iterator(['page_view', 'click', 'form_submit', 'download', 'search'])
    event_data = factory.LazyAttribute(lambda obj: {
        'page_view': {'url': fake.url(), 'title': fake.sentence()},
        'click': {'element': fake.word(), 'position': {'x': fake.random_int(0, 1920), 'y': fake.random_int(0, 1080)}},
        'form_submit': {'form_id': fake.word(), 'fields': fake.random_int(1, 10)},
        'download': {'filename': fake.file_name(), 'size': fake.random_int(1024, 10485760)},
        'search': {'query': fake.sentence(nb_words=3), 'results': fake.random_int(0, 100)}
    }.get(obj.event_type, {}))
...
class PageViewFactory(BaseFactory):
    ...
    time_on_page = factory.LazyAttribute(lambda obj: fake.random_int(5, 600))  # 5 seconds to 10 minutes
    timestamp = factory.LazyFunction(timezone.now)