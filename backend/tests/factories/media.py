...
class MediaItemFactory(BaseFactory):
    ...
    mime_type = factory.LazyAttribute(lambda obj: {
        'image': 'image/jpeg',
        'video': 'video/mp4', 
    ...
    file_size = factory.LazyAttribute(lambda obj: fake.random_int(min=1024, max=10485760))  # 1KB to 10MB
    
    # Generate realistic filename
    original_filename = factory.LazyAttribute(lambda obj: 
        f"{fake.slug()}.{obj.mime_type.split('/')[-1]}"
    )
    
    uploaded_by = factory.SubFactory(UserFactory)
    
    # Generate file content (simplified for testing)
    file = factory.LazyAttribute(lambda obj: 
        ContentFile(b'test file content', name=obj.original_filename)
    )
    ...
    metadata = factory.LazyAttribute(lambda obj: {
        ...
        'document': {
            'pages': fake.random_int(min=1, max=100)
        },
        'audio': {
