...
class FilesModelTests(TestCase):
    ...
    def test_file_category_methods(self):
        """Test FileCategory model methods."""
        self.assertEqual(str(self.category), 'Test Category')
        
        # Test get_file_count method
        File.objects.create(name='file1.pdf', uploaded_by=self.user, category=self.category)
        File.objects.create(name='file2.pdf', uploaded_by=self.user, category=self.category)
    ...
    def test_file_processing_service(self):
        ...
        try:
            ...
            if hasattr(processing_service, 'process_image'):
                processed = processing_service.process_image(
                    image, 
                    width=50, 
                    height=50
                )
