...
class PageRevisionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = PageRevision
        fields = [
            'id', 'created_at', 'created_by_email', 'created_by_name',
            'is_published_snapshot', 'is_autosave', 'comment', 
            'block_count', 'revision_type'
        ]
...
class AuditEntrySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = AuditEntry
        fields = [
            'id', 'actor_email', 'actor_name', 'action', 'model_label',
            'object_id', 'object_name', 'created_at', 'ip_address', 'meta'
        ]
        read_only_fields = ['id', 'created_at']
...
class AutosaveSerializer(serializers.Serializer):
    ...
    comment = serializers.CharField(
        required=False,
        allow_blank=True,
        help_text="Optional comment for the autosave"
    )
...
class PublishPageSerializer(serializers.Serializer):
    ...
    published_at = serializers.DateTimeField(
        required=False,
        help_text="When to publish the page. Defaults to now."
    )
    comment = serializers.CharField(
        required=False,
        allow_blank=True,
