...
class PresentationPageResolver:
    ...
    def resolve_by_id(self, content_label: str, content_id: int, locale_code: str) -> Dict[str, Any]:
        """
        ...
        if content_label == 'blog.blogpost':
            ...
            try:
                post = BlogPost.objects.select_related(
                    'category', 'locale'
                ).get(id=content_id, locale__code=locale_code)
                
                return self._resolve_blog_post_from_instance(post)
            except BlogPost.DoesNotExist:
                raise Http404(f"BlogPost {content_id} not found for locale {locale_code}")
    ...
    def _resolve_blog_post(self, slug: str, locale_code: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise Http404(f"Published blog post '{slug}' not found for locale {locale_code}")
    ...
    def validate_content_detail_block(self, blocks: list, allowed_labels: Optional[list] = None) -> None:
        """
        ...
        if len(content_detail_blocks) == 0:
            raise ValidationError("Presentation pages must include exactly one content_detail block")
        
        if len(content_detail_blocks) > 1:
            raise ValidationError("Presentation pages can only have one content_detail block")
        ...
        if allowed_labels and label not in allowed_labels:
            raise ValidationError(f"content_detail block label '{label}' not in allowed list: {allowed_labels}")
...
def resolve_presentation_page(path: str = None, content_label: str = None, 
                            content_id: int = None, locale_code: str = 'en') -> Dict[str, Any]:
    """
    ...
    if path:
        ...
        return presentation_resolver.resolve_by_id(content_label, content_id, locale_code)
    else:
