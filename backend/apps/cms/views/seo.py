...
class SeoSettingsViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        include_defaults = (
            request.query_params.get("include_defaults", "false").lower() == "true"
        )
    ...
    def retrieve(self, request, pk=None):
        ...
        try:
            ...
            logger.debug(f"Failed to retrieve SEO settings by pk {pk}: {e}")
    ...
    def by_locale(self, request):
        ...
        if not locale_code:
            ...
            return Response(
                {"error": "locale code is required"}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def preview(self, request):
        ...
        try:
            ...
            default_og_image = (
                settings.default_og_asset.url if settings.default_og_asset else ""
            )
