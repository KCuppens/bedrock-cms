...
class PagesViewSet(VersioningMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return Page.objects.select_related('locale', 'parent').annotate(
    ...
    @extend_schema(
        parameters=[
            ...
            OpenApiParameter('preview', str, description='Preview token UUID for draft pages'),
        ],
        responses={
    ...
    def get_by_path(self, request):
        ...
        try:
            ...
            locale = Locale.objects.select_related().get(code=locale_code, is_active=True)
        except Locale.DoesNotExist:
            return Response({'error': f'Locale "{locale_code}" not found or inactive'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if page.status != 'published':
            if page.status == 'draft':
                if preview_token:
                    ...
                    if not request.user.is_authenticated or not request.user.has_perm('cms.preview_page'):
                        return Response({'error': 'Page not published'}, status=status.HTTP_404_NOT_FOUND)
            else:
                # For scheduled, pending_review, etc - only show if authenticated with permission
                if not request.user.is_authenticated or not request.user.has_perm('cms.view_page'):
                    return Response({'error': 'Page not available'}, status=status.HTTP_404_NOT_FOUND)
        ...
        if page.status == 'published':
            ...
            if page.updated_at:
                response['Last-Modified'] = page.updated_at.strftime('%a, %d %b %Y %H:%M:%S GMT')
        else:
    ...
    def move(self, request, pk=None):
        ...
        with transaction.atomic():
            ...
            siblings = Page.objects.filter(
                ...
                locale=page.locale
            ).exclude(pk=page.pk).order_by('position', 'id')
            ...
            for index, sibling in enumerate(siblings_list):
                ...
                sibling.save(update_fields=['position'] if sibling.pk != page.pk else ['parent', 'position'])
    ...
    @extend_schema(
        request={
            'application/json': {
                'type': 'object',
                'properties': {
                    'parent_id': {'type': ['integer', 'null'], 'description': 'Parent page ID, null for root pages'},
                    'page_ids': {
                        'type': 'array',
                        'items': {'type': 'integer'},
    ...
    def reorder(self, request):
        ...
        with transaction.atomic():
            ...
            if parent_id is None:
                pages_filter = pages_query.filter(
                    id__in=page_ids,
                    parent__isnull=True
                )
            else:
            ...
            cases = [When(id=page_id, then=Value(position)) 
            ...
            all_siblings = Page.objects.filter(
                parent_id=parent_id if parent_id is not None else None
            ).exclude(id__in=page_ids).order_by('position', 'id')
    ...
    def update_block(self, request, pk=None, block_index=None):
        ...
        if block_index is None:
            return Response({'error': 'block_index is required'}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def duplicate(self, request, pk=None):
        ...
        try:
            ...
            duplicated_page = Page.objects.select_related('locale').get(pk=duplicated_page.pk)
            return Response(PageReadSerializer(duplicated_page).data, status=status.HTTP_201_CREATED)
        except Exception as e:
            return Response({'error': str(e)}, status=status.HTTP_400_BAD_REQUEST)
