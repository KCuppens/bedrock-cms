...
class TagViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if min_count:
            queryset = queryset.annotate(
                post_count=Count('posts')
            ).filter(post_count__gte=int(min_count))
    ...
    def trending(self, request):
        ...
        limit = int(request.query_params.get('limit', 10))
        tags = self.queryset.filter(
            posts__created_at__gte=thirty_days_ago
        ).annotate(
            post_count=Count('posts'),
            recent_post_count=Count('posts', filter=Q(posts__created_at__gte=thirty_days_ago))
        ).filter(recent_post_count__gt=0).order_by('-recent_post_count')[:limit]
    ...
    def unused(self, request):
        ...
        unused_tags = self.queryset.annotate(
            post_count=Count('posts')
...
@extend_schema_view(
    ...
    partial_update=extend_schema(summary="Partially update collection", tags=["Collections"]),
