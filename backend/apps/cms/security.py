...
def get_sanitization_config():
    ...
    """
    return {
        'allowed_tags': getattr(settings, 'HTML_SANITIZER_ALLOWED_TAGS', DEFAULT_ALLOWED_TAGS),
        'allowed_attributes': getattr(settings, 'HTML_SANITIZER_ALLOWED_ATTRIBUTES', DEFAULT_ALLOWED_ATTRIBUTES),
        'allowed_protocols': getattr(settings, 'HTML_SANITIZER_ALLOWED_PROTOCOLS', DEFAULT_ALLOWED_PROTOCOLS),
    }
                ...
                for item in props['items']:
                    if isinstance(item, dict):
                        ...
                        for field in ['question', 'answer']:
                            if field in sanitized_item and isinstance(sanitized_item[field], str):
                                sanitized_item[field] = sanitize_html(sanitized_item[field])
