...
class PagesViewSet(VersioningMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = Page.objects.select_related(
            ...
            Prefetch('children', queryset=Page.objects.only('id', 'title', 'slug', 'status'))
        ).annotate(
    ...
    def get_permissions(self):
        ...
        elif self.action in ['approve', 'reject', 'moderation_queue', 'moderation_stats']:
    ...
    @extend_schema(
        parameters=[
            ...
            OpenApiParameter('with_seo', str, description='Include resolved SEO data (1 to enable)'),
        ]
    )
        ...
        if page.status == 'draft':
            if preview_token:
                ...
                if str(page.preview_token) != preview_token:
                    return Response({'error': 'Invalid preview token'}, status=status.HTTP_403_FORBIDDEN)
            else:
    ...
    def tree(self, request):
        ...
        if root_id:
            try:
                ...
                return Response({'error': 'Root page not found'}, status=status.HTTP_404_NOT_FOUND)
        else:
    ...
    def reorder_children(self, request, pk=None):
        ...
        with transaction.atomic():
            ...
            cases = [When(id=child_id, then=Value(position)) 
    ...
    def schedule(self, request, pk=None):
        ...
        if not publish_at_str:
            return Response({'error': 'publish_at is required'}, status=status.HTTP_400_BAD_REQUEST)
        
        try:
            # Parse the datetime strings
            publish_at = parse_datetime(publish_at_str)
            if not publish_at:
                return Response({'error': 'Invalid publish_at format. Use ISO format.'}, status=status.HTTP_400_BAD_REQUEST)
            
            unpublish_at = None
            if unpublish_at_str:
                unpublish_at = parse_datetime(unpublish_at_str)
                if not unpublish_at:
                    return Response({'error': 'Invalid unpublish_at format. Use ISO format.'}, status=status.HTTP_400_BAD_REQUEST)
            ...
            return Response({'error': f'Failed to schedule page: {str(e)}'}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def unschedule(self, request, pk=None):
        ...
        try:
            ...
            return Response({
                'status': page.status,
                'message': 'Page unscheduled successfully'
            })
        except Exception as e:
            return Response({'error': f'Failed to unschedule page: {str(e)}'}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def schedule_unpublish(self, request, pk=None):
        ...
        if not unpublish_at_str:
            return Response({'error': 'unpublish_at is required'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            return Response({'error': f'Failed to schedule unpublishing: {str(e)}'}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def scheduled_content(self, request):
        ...
        scheduled_pages = self.get_queryset().filter(
            status='scheduled',
    ...
    def moderation_queue(self, request):
        ...
        pending_pages = self.get_queryset().filter(
            status='pending_review'
    ...
    def moderation_stats(self, request):
        ...
        stats = {
            'pending_review': self.get_queryset().filter(status='pending_review').count(),
        ...
        reviewer_stats = self.get_queryset().filter(
    ...
    def update_block(self, request, pk=None, block_index=None):
        ...
        try:
            ...
            if index < 0 or index >= len(page.blocks):
                return Response({'error': 'Block index out of range'}, status=status.HTTP_400_BAD_REQUEST)
        except ValueError:
    ...
    def insert_block(self, request, pk=None):
        ...
        if at_index < 0 or at_index > len(page.blocks):
            return Response({'error': 'Invalid insertion index'}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def reorder_blocks(self, request, pk=None):
        ...
        if from_index is None or to_index is None:
            return Response({'error': 'from and to indices are required'}, status=status.HTTP_400_BAD_REQUEST)

        if (from_index < 0 or from_index >= len(page.blocks) or 
            to_index < 0 or to_index >= len(page.blocks)):
            return Response({'error': 'Block indices out of range'}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def presentation_templates(self, request):
        ...
        if not content_type:
            return Response({'error': 'content_type parameter required'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        return Response({
            'templates': PageReadSerializer(template_pages, many=True, context={'request': request}).data,
    ...
    def resolve_content(self, request):
        ...
        if len(path_parts) < 2:
            return Response({'error': 'Invalid path format'}, status=status.HTTP_404_NOT_FOUND)
        ...
        return Response({'error': 'Content type not supported'}, status=status.HTTP_404_NOT_FOUND)
    ...
    def _resolve_blog_post(self, slug, locale_code, request):
        ...
        elif post.category and hasattr(post.category, 'presentation_page') and post.category.presentation_page:
            presentation_page = post.category.presentation_page
            resolution_source = 'category'
        # 3. Check global default
        else:
            try:
                settings = BlogSettings.objects.select_related('default_presentation_page').get(locale=locale)
        ...
        response_data = {
            ...
            'presentation_page': PageReadSerializer(presentation_page, context={'request': request}).data if presentation_page else None,
...
def sitemap_view(request, locale_code):
    ...
    pages = Page.objects.filter(
        locale=locale, 
        status='published'
    ).order_by('updated_at')[:50000].iterator(chunk_size=1000)
