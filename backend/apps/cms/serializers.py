...
class PageReadSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Page
        fields = [
            'id', 'group_id', 'locale', 'parent', 'position', 'path',
            'title', 'slug', 'status', 'published_at', 'scheduled_publish_at',
            'scheduled_unpublish_at', 'updated_at', 'blocks', 'seo', 
            'children_count', 'resolved_seo', 'seo_links', 'recent_revisions'
        ]
    ...
    def get_recent_revisions(self, obj):
        ...
        print(f"DEBUG: Returning {len(mock_revisions)} mock revisions from serializers.py")
...
class PageWriteSerializer(serializers.ModelSerializer):
    class Meta:
        model = Page
        fields = [
            'parent', 'locale', 'title', 'slug', 'status', 
    ...
    def validate_scheduled_unpublish_at(self, value):
        ...
        if value:
            from django.utils import timezone
            if value <= timezone.now():
                raise serializers.ValidationError("Scheduled unpublish time must be in the future")
        return value
...
class SeoSettingsSerializer(serializers.ModelSerializer):
    class Meta:
        model = SeoSettings
        fields = [
            'id', 'locale', 'title_suffix', 'default_description',
            'robots_default', 'jsonld_default', 'created_at', 'updated_at'
