...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not block_type.replace('_', '').isalnum():
            raise CommandError('Block type name must contain only letters, numbers, and underscores')
        ...
        self.stdout.write(
            self.style.SUCCESS(f'Creating new block type: {block_type}')
        )
    ...
    def _parse_properties(self, props):
        ...
        for prop in props:
            parts = prop.split(':', 2)
            if len(parts) < 2:
                raise CommandError(f'Invalid property format: {prop}. Use "name:type:default"')
    ...
    def _show_dry_run(self, context):
        ...
        self.stdout.write(f'  * tests/unit/test_blocks_{context["block_type"]}.py (new)')
    ...
    def _update_validation_file(self, context):
        ...
        new_content = parts[0] + block_model + '\n\n# Registry of all block models\n' + registry_line + parts[1]
        
        # Add to registry
        registry_entry = f'    "{context["block_type"]}": {context["block_class"]}BlockModel,'
        registry_end = '}'
        ...
        new_content = registry_parts[0] + registry_entry + '\n' + registry_end + registry_end.join(registry_parts[1:])
...
''')
    ...
    def _create_docs_file(self, context):
        ...
        template = Template('''# {{ block_title }} Block
...
''')
    ...
    def _show_next_steps(self, context):
        ...
        self.stdout.write(f'   - Review tests in tests/unit/test_blocks_{context["block_type"]}.py')
        self.stdout.write(f'   - Update documentation in docs/blocks/{context["block_type"]}.md')
        
        self.stdout.write('\n2. Run the tests:')
        self.stdout.write(f'   python manage.py test tests.unit.test_blocks_{context["block_type"]}')
        
        self.stdout.write('\n3. Test the block validation:')
        self.stdout.write('   python manage.py shell')
        self.stdout.write('   >>> from apps.cms.blocks.validation import validate_blocks')
        self.stdout.write(f'   >>> blocks = [{{"type": "{context["block_type"]}", "props": {{}}}}]')
