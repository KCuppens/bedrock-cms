...
from .versioning_serializers import (
    PageRevisionSerializer, PageRevisionDetailSerializer, RevisionDiffSerializer,
    RevertRevisionSerializer, AuditEntrySerializer, AutosaveSerializer,
    PublishPageSerializer, UnpublishPageSerializer
)
...
class PageRevisionViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    @extend_schema(
        ...
        parameters=[
            OpenApiParameter('against', str, description='ID of revision to compare against')
        ],
    ...
    def diff(self, request, pk=None):
        ...
        try:
            against_revision = PageRevision.objects.get(id=against_id, page=revision.page)
    ...
    def revert(self, request, pk=None):
        ...
        return Response({'message': f'Page reverted to revision from {revision.created_at}'})
...
class AuditEntryViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    queryset = AuditEntry.objects.select_related('actor').prefetch_related('content_object')
...
class VersioningMixin:
    ...
    @extend_schema(
        ...
        responses={201: {'type': 'object', 'properties': {'revision_id': {'type': 'string'}}}}
    )
    @action(detail=True, methods=['post'])
    ...
    def publish(self, request, pk=None):
        ...
        page.published_at = serializer.validated_data.get('published_at', timezone.now())
        
        # Set context for signals
        page._current_user = request.user
        page._current_request = request
        page._revision_comment = serializer.validated_data.get('comment', 'Page published')
    ...
    def unpublish(self, request, pk=None):
        ...
        page._current_user = request.user
        page._current_request = request
        page._revision_comment = serializer.validated_data.get('comment', 'Page unpublished')
