...
class LinkExtractor:
    ...
    def __init__(self, base_url: str = 'http://localhost:8000'):
        ...
        self.session.headers.update({
            'User-Agent': 'Bedrock-CMS-LinkChecker/1.0'
        })
    
    def extract_links_from_blocks(self, blocks: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    def _extract_links_from_block(self, block: Dict[str, Any], block_index: int, 
        ...
        block_path = f"{parent_path}blocks[{block_index}]" if parent_path else f"blocks[{block_index}]"
...
def check_internal_links(self, page_ids: Optional[List[int]] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info("Link check completed: %s links checked, {len(results['broken_links'])} broken", results['total_links_checked'])
        return results
...
def publish_scheduled_content():
    ...
    try:
        ...
        for page in scheduled_pages:
            try:
                with transaction.atomic():
                    ...
                    published_count += 1
                    logger.info(f"Published scheduled page: {page.title} (ID: {page.id})")
            except Exception as e:
            ...
            for post in scheduled_posts:
                try:
                    with transaction.atomic():
                        ...
                        published_count += 1
                        logger.info(f"Published scheduled blog post: {post.title} (ID: {post.id})")
                except Exception as e:
        ...
        if published_count > 0 or errors:
            logger.info(f"Scheduled publishing completed: {published_count} items published, {len(errors)} errors")
    ...
    try:
        ...
        for task in pending_tasks:
            try:
                ...
                if success:
                    processed_count += 1
                    logger.info(f"Successfully executed task {task.id}: {task.task_type} for {task.content_object}")
                else:
                    failed_count += 1
                    errors.append(f"Task {task.id} failed: {task.error_message}")
                    logger.error(f"Failed to execute task {task.id}: {task.error_message}")
        ...
        logger.info(f"Scheduled publishing completed: {processed_count} processed, {failed_count} failed")
