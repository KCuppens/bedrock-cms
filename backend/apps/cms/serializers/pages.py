...
class PageTreeItemSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Page
        fields = [
            'id', 'title', 'slug', 'path', 'status',
            'locale', 'locale_name', 'parent', 'position',
            'created_at', 'updated_at', 'children_count',
            'in_main_menu', 'in_footer', 'is_homepage'
        ]
...
class PageReadSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Page
        fields = [
            'id', 'group_id', 'parent', 'position', 'locale', 
            'title', 'slug', 'path', 'blocks', 'seo',
            'status', 'published_at', 'created_at', 'updated_at',
            'preview_token', 'children', 'updated_by', 'updated_by_name',
            'resolved_seo', 'seo_links', 'in_main_menu', 'in_footer', 'is_homepage',
            'recent_revisions'
        ]
        read_only_fields = [
            'id', 'group_id', 'path', 'created_at', 'updated_at', 'preview_token',
    ...
    def get_updated_by(self, obj):
        ...
        try:
            latest_revision = obj.revisions.select_related('created_by').order_by('-created_at').first()
...
class PageWriteSerializer(serializers.ModelSerializer):
    ...
    slug = serializers.CharField(required=False, allow_blank=True)  # Allow empty slug for homepage
    
    class Meta:
        model = Page
        fields = [
            'parent', 'position', 'locale', 'title', 'slug',
            'blocks', 'seo', 'status', 'published_at',
            'in_main_menu', 'in_footer', 'is_homepage'
        ]
    ...
    def validate_locale(self, value):
        ...
        try:
            ...
            try:
                ...
                raise serializers.ValidationError(f"Locale with code '{value}' does not exist.")
    ...
    def validate_slug(self, value):
        ...
        parent = self.initial_data.get('parent') or (instance.parent if instance else None)
        locale_code = self.initial_data.get('locale') or (instance.locale.code if instance else None)
        
        # Convert locale code to locale instance if needed
        from apps.i18n.models import Locale
        if locale_code:
            try:
                locale = Locale.objects.get(code=locale_code.lower()) if isinstance(locale_code, str) else locale_code
            except Locale.DoesNotExist:
                # Skip validation if locale doesn't exist (will be caught by locale field validation)
                return value
        else:
            locale = None
        ...
        if queryset.exists():
            raise serializers.ValidationError("A page with this slug already exists at this location.")
