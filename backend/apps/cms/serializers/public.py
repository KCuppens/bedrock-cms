...
class PublicPageSerializer(serializers.ModelSerializer):
    ...
    def get_resolved_seo(self, obj):
        ...
        try:
            ...
            base_url = getattr(self.context.get('request'), 'build_absolute_uri', lambda x: x)('/')
            ...
            return {
                ...
                'robots': 'noindex,nofollow' if obj.status == 'draft' else 'index,follow',
                'canonical_url': self.context.get('request').build_absolute_uri(obj.path) if self.context.get('request') else obj.path,
                'locale_code': obj.locale.code,
            }
