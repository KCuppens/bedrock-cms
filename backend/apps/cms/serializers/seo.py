...
class SeoSettingsSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = ['id', 'locale_code', 'locale_name', 'created_at', 'updated_at']
    ...
    def get_default_og_image_url(self, obj):
        ...
        if obj.default_og_asset:
            ...
            if request:
                return request.build_absolute_uri(f"/api/v1/files/{obj.default_og_asset.id}/download/")
        return None
    ...
    def get_default_twitter_image_url(self, obj):
        ...
        if obj.default_twitter_asset:
            ...
            # Fallback to building URL manually
            if request:
                return request.build_absolute_uri(f"/api/v1/files/{obj.default_twitter_asset.id}/download/")
        return None
    ...
    def validate_robots_default(self, value):
        ...
        valid_directives = ['index', 'noindex', 'follow', 'nofollow', 'none', 'noarchive', 'nosnippet']
        if value:
            parts = [p.strip() for p in value.lower().split(',')]
            for part in parts:
                if part not in valid_directives:
                    raise serializers.ValidationError(f"Invalid robots directive: {part}")
        return value
