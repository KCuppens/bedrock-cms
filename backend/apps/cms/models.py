...
class Page(models.Model, RBACMixin):
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=['locale', 'parent', 'slug'],
                name='uq_page_slug_parent_locale'
            )
        ]
        indexes = [
            models.Index(fields=['group_id', 'locale']),
            models.Index(fields=['locale', 'path']),  # Composite index for get_by_path
            models.Index(fields=['locale', 'status', '-updated_at']),  # For sitemap queries
            models.Index(fields=['parent', 'position']),  # For tree navigation
            models.Index(fields=['status', 'submitted_for_review_at']),  # For moderation queue
    ...
    def clean(self):
        ...
        if self.status == 'published' and self.scheduled_publish_at:
            errors['scheduled_publish_at'] = _('Cannot schedule publishing for already published content')
        
        # Rule 3: Unpublish scheduling only for published content
        if self.scheduled_unpublish_at:
            if self.status != 'published':
                errors['scheduled_unpublish_at'] = _('Can only schedule unpublishing for published content')
        ...
        if self.scheduled_publish_at and self.scheduled_unpublish_at:
            if self.scheduled_unpublish_at <= self.scheduled_publish_at:
                errors['scheduled_unpublish_at'] = _('Must be after scheduled publish time')
    ...
    def save(self, *args, **kwargs):
        ...
        if self.pk:
            ...
            if (old_instance.slug != self.slug or 
                old_instance.parent_id != self.parent_id or 
                old_instance.locale_id != self.locale_id):
                self.path = self.compute_path()
                # Need to update descendants if parent or slug changed
                if old_instance.parent_id != self.parent_id or old_instance.slug != self.slug:
    ...
    def _validate_presentation_page_blocks(self):
        ...
        if is_presentation_page:
            presentation_resolver.validate_content_detail_block(
                self.blocks or [],
                allowed_labels=['blog.blogpost']  # Could be extended for other content types
            )
    ...
    def submit_for_review(self, user=None):
        ...
        if self.status not in ['draft', 'rejected']:
            raise ValidationError("Page must be in draft or rejected status to submit for review")
        ...
        self.save(update_fields=['status', 'submitted_for_review_at', 'review_notes', 'reviewed_by'])
...
class BlockType(models.Model):
    ...
    schema = models.JSONField(
        default=dict,
        help_text=_('JSON schema for validating block props')
    )
    ...
    def clean(self):
        ...
        if self.component and not self.component.endswith('Block'):
            raise ValidationError({
                'component': _('Component name should end with "Block" (e.g., "HeroBlock")')
            })
        
        # Validate icon name (basic check)
        if self.icon and not self.icon.replace('-', '').replace('_', '').isalnum():
            raise ValidationError({
                'icon': _('Icon name should only contain letters, numbers, hyphens, and underscores')
            })
    ...
    def get_registry_dict(cls):
        ...
        if registry is None:
            block_types = cls.objects.filter(is_active=True).values(
                'type', 'component', 'label', 'description', 'category', 
                'icon', 'preload', 'editing_mode', 'schema', 'default_props',
                'model_name', 'data_source', 'api_endpoint', 'query_schema'
            )
