...
class LocaleModelTest(TestCase):
    ...
    def test_default_locale_constraint(self):
        ...
        Locale.objects.create(code='en', name='English', native_name='English', is_default=True)
        locale2 = Locale.objects.create(code='es', name='Spanish', native_name='Español', is_default=True)
...
class PageModelTest(TestCase):
    ...
    def setUp(self):
        self.locale = Locale.objects.create(code='en', name='English', native_name='English')
    ...
    def test_reparent_updates_descendant_paths(self):
        ...
        products = Page.objects.create(title='Products', slug='products', locale=self.locale)
        software = Page.objects.create(title='Software', slug='software', parent=products, locale=self.locale)
        features = Page.objects.create(title='Features', slug='features', parent=software, locale=self.locale)
        
        # Create new parent
        solutions = Page.objects.create(title='Solutions', slug='solutions', locale=self.locale)
    ...
    def test_siblings_resequence(self):
        ...
        Page.objects.create(title='Child 1', slug='child1', parent=parent, position=0, locale=self.locale)
        Page.objects.create(title='Child 2', slug='child2', parent=parent, position=5, locale=self.locale)
        Page.objects.create(title='Child 3', slug='child3', parent=parent, position=10, locale=self.locale)
    ...
    def test_unique_constraint(self):
        ...
        parent = Page.objects.create(title='Parent', slug='parent', locale=self.locale)
        Page.objects.create(title='Child 1', slug='child', parent=parent, locale=self.locale)
        
        # Creating another child with same slug should fail
        with self.assertRaises(Exception):
            Page.objects.create(title='Child 2', slug='child', parent=parent, locale=self.locale)
    
    def test_same_slug_different_parent_allowed(self):
        """Test same slug under different parents is allowed."""
        parent1 = Page.objects.create(title='Parent 1', slug='parent1', locale=self.locale)
        parent2 = Page.objects.create(title='Parent 2', slug='parent2', locale=self.locale)
        
        # Same slug under different parents should work
        Page.objects.create(title='Child 1', slug='child', parent=parent1, locale=self.locale)
        Page.objects.create(title='Child 2', slug='child', parent=parent2, locale=self.locale)
...
class PagesAPITest(APITestCase):
    ...
    def setUp(self):
        self.locale = Locale.objects.create(code='en', name='English', native_name='English', is_default=True)
    ...
    def test_get_page_by_path_with_preview_token(self):
        ...
        response = self.client.get(url, {
            'path': '/home',
            'locale': 'en',
            'preview': str(self.page.preview_token)
        })
    ...
    def test_block_editing(self):
        ...
        # Add a block
        self.page.blocks = [
            {'type': 'hero', 'props': {'title': 'Original Title'}}
        ]
        self.page.save()
    ...
    def test_tree_endpoint(self):
        ...
        products_data = next(item for item in response.data if item['slug'] == 'products')
...
class SitemapTest(TestCase):
    ...
    def setUp(self):
        self.locale = Locale.objects.create(code='en', name='English', native_name='English', is_active=True)
        Page.objects.create(
            title='Home',
            slug='home',
            locale=self.locale,
...
class SeoUtilsTest(TestCase):
    ...
    def setUp(self):
        self.locale = Locale.objects.create(code='en', name='English', native_name='English', is_default=True)
        self.locale_fr = Locale.objects.create(code='fr', name='French', native_name='Français')
...
class SeoAPITest(APITestCase):
    ...
    def test_page_api_with_seo(self):
        ...
        response = self.client.get(url, {'path': '/test', 'locale': 'en', 'with_seo': '1'})
...
class SitemapEnhancedTest(TestCase):
    ...
    def setUp(self):
        self.locale_en = Locale.objects.create(code='en', name='English', native_name='English', is_active=True)
        self.locale_fr = Locale.objects.create(code='fr', name='French', native_name='Français', is_active=True)
