            ...
            try:
                ...
                if not cache.get(cache_key):
                    ...
                    if getattr(settings, 'CELERY_TASK_ALWAYS_EAGER', False):
                        ...
                        logger.debug("Updated last_seen directly for user %s", request.user.id)
                    else:
                        # In production, use Celery task
                        try:
                            from .tasks import update_user_last_seen
                            update_user_last_seen.delay(request.user.id)
                            logger.debug("Queued last_seen update for user %s", request.user.id)
                            ...
                            logger.debug("Celery unavailable, updated last_seen directly for user %s", request.user.id)
