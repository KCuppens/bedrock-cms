...
class ScopedPermissionModelsTests(TestCase):
    ...
    def test_scoped_locale_creation(self):
        ...
        self.assertEqual(str(scoped_locale), f"{self.editors_group.name} → {self.en_locale.name}")
...
class ScopedPermissionBackendTests(TestCase):
    ...
    def setUp(self):
        ...
        ScopedLocale.objects.create(group=self.blog_editors_group, locale=self.en_locale)
    ...
    def test_user_with_scope_access(self):
        ...
        try:
            result1 = self.backend.has_perm(self.user, 'cms.change_page', self.en_home_page)
            result2 = self.backend.has_perm(self.user, 'cms.change_page', self.en_blog_page)  
            result3 = self.backend.has_perm(self.user, 'cms.change_page', self.es_home_page)
    ...
    def test_user_with_restricted_scope(self):
        ...
        try:
            result = self.backend.has_perm(self.restricted_user, 'cms.change_page', self.en_blog_page)
        ...
        try:
            result = self.backend.has_perm(self.restricted_user, 'cms.change_page', self.en_home_page)
        ...
        try:
            result = self.backend.has_perm(self.restricted_user, 'cms.change_page', self.es_home_page)
            self.assertIsNotNone(result)  # Just verify it completes
        except Exception:
            self.fail("Permission check should complete without exception")
    ...
    def test_get_user_scoped_sections(self):
        ...
        blog_editor_sections = self.backend.get_user_scoped_sections(self.restricted_user)
        ...
        for user, page, expected in test_cases:
            with self.subTest(user=user.email, page=page.title):
                ...
                try:
                    ...
                    self.fail(f"Permission check failed for {user.email} -> {page.title}: {e}")