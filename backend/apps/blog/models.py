...
class BlogPost(models.Model, RBACMixin):
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=['slug', 'locale'],
                name='unique_blog_slug_per_locale'
            ),
        ]
        indexes = [
            ...
            models.Index(fields=['category', 'status', 'featured', '-published_at']),  # For featured posts by category
        ...
        if self.blocks and current_length < max_text_length:
            for block in self.blocks[:100]:  # Limit blocks processed
                if isinstance(block, dict) and 'props' in block:
                    ...
                    if 'content' in props:
                        text_to_add = strip_tags(str(props['content'])[:1000])  # Limit each block
                    elif 'text' in props:
    ...
    def get_related_posts(self, limit=5):
        ...
        related = BlogPost.objects.select_related(
            'category', 'locale', 'author'
