...
class BlogPostAdmin(admin.ModelAdmin):
    ...
    list_filter = (
        'status', 'featured', 'category', 'tags', 'allow_comments',
        'created_at', 'published_at', 'locale'
    )
    search_fields = ('title', 'excerpt', 'content', 'author__email')
    prepopulated_fields = {'slug': ('title',)}
    filter_horizontal = ('tags',)
    readonly_fields = (
        'group_id', 'created_at', 'updated_at', 'reading_time_display',
    ...
    fieldsets = (
        ...
        ('Content', {
            'fields': ('excerpt', 'content', 'blocks', 'tags')
        }),
        ('Publishing', {
            'fields': ('status', 'featured', 'allow_comments', 'published_at', 'scheduled_for')
        }),
        ('SEO', {
            'fields': ('seo', 'social_image'),
            'classes': ('collapse',)
        }),
        ('Metadata', {
            'fields': ('group_id', 'reading_time_display', 'created_at', 'updated_at'),
            'classes': ('collapse',)
        }),
    )
    ...
    def get_queryset(self, request):
        ...
        return super().get_queryset(request).select_related(
            'author', 'category', 'locale', 'social_image'
...
class BlogSettingsAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Display Options', {
            'fields': (
                'show_toc', 'show_author', 'show_dates', 
                'show_share', 'show_reading_time'
            )
        }),
        ('Design & SEO', {
            'fields': ('design_tokens', 'seo_defaults'),
            'classes': ('collapse',)
        }),
        ('Feeds', {
            'fields': ('feeds_config',),
