...
class BlogPostViewSet(viewsets.ModelViewSet):
    ...
    def perform_update(self, serializer):  # noqa: C901
        ...
        BlogPostRevision.create_snapshot(
            blog_post=blog_post, user=self.request.user, comment="Manual update"
        )
    ...
    def unpublish(self, request, pk=None):  # noqa: C901
        ...
        # Create revision
        BlogPostRevision.create_snapshot(
            blog_post=blog_post, user=request.user, comment="Unpublished via API"
        )
    ...
    @extend_schema(
        ...
        parameters=[
            OpenApiParameter("revision_id", str, description="Revision ID to revert to")
        ],
        responses={200: BlogPostSerializer},
    )
    ...
    def revert(self, request, pk=None, revision_id=None):  # noqa: C901
        ...
        try:
            revision = BlogPostRevision.objects.get(id=revision_id, blog_post=blog_post)
...
def blog_settings_api(request, locale_code=None):  # noqa: C901
    ...
    if locale_code:
        ...
        if not locale:
            return Response(
                {"error": "No active locales found"}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    if request.method == "GET":
        ...
        try:
            ...
            serializer = BlogSettingsSerializer(
