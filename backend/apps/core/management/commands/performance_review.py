...
class Command(BaseCommand):
    ...
    def analyze_database_queries(self, quick_mode=False):
        ...
        pages_optimized = list(Page.objects.select_related('parent', 'locale')
                             .prefetch_related('children').all()[:5])
        ...
        posts_optimized = list(BlogPost.objects.select_related('category', 'locale')
                             .prefetch_related('tags').all()[:5])
        ...
        self.results['database'] = {
            'page_tree': {
                ...
                'optimized': {'time': tree_optimized_time, 'queries': tree_optimized_queries},
                'improvement': f"{((tree_time - tree_optimized_time) / tree_time * 100):.1f}%" if tree_time > 0 else "N/A"
            },
            'blog_posts': {
                'naive': {'time': blog_time, 'queries': blog_queries},
                'optimized': {'time': blog_optimized_time, 'queries': blog_optimized_queries},
                'improvement': f"{((blog_time - blog_optimized_time) / blog_time * 100):.1f}%" if blog_time > 0 else "N/A"
            }
        }
        
        self.stdout.write(f"    * Page tree: {tree_queries} -> {tree_optimized_queries} queries")
        self.stdout.write(f"    * Blog posts: {blog_queries} -> {blog_optimized_queries} queries")
    ...
    def analyze_api_performance(self, quick_mode=False):
        ...
        for method, endpoint, name in endpoints:
            ...
            if times:
                ...
                status_icon = "[OK]" if avg_time < 100 else "[WARN]" if avg_time < 500 else "[CRITICAL]"
                self.stdout.write(f"    {status_icon} Avg: {avg_time:.1f}ms, Range: {min_time:.1f}-{max_time:.1f}ms")
    ...
    def analyze_caching(self):
        ...
        test_data = {'test': 'performance_data', 'timestamp': timezone.now().isoformat()}
        ...
        self.stdout.write(f"    * Backend: {cache_results['cache_backend'].split('.')[-1]}")
        self.stdout.write(f"    * Set: {set_time:.2f}ms, Get: {get_time:.2f}ms, Delete: {delete_time:.2f}ms")
        ...
        for count in test_cases:
            ...
            if times:
                ...
                block_results[count] = {
                    ...
                    'status': 'good' if avg_time < 200 else 'warning' if avg_time < 1000 else 'critical'
                }
                
                status_icon = "[OK]" if avg_time < 200 else "[WARN]" if avg_time < 1000 else "[CRITICAL]"
                self.stdout.write(f"      {status_icon} {avg_time:.1f}ms total, {avg_time/count:.2f}ms per block")
    ...
    def analyze_media_performance(self):
        ...
        media_results = {
            'storage_backend': str(getattr(settings, 'DEFAULT_FILE_STORAGE', 'django.core.files.storage.FileSystemStorage')),
            'operations': {}
        }
        ...
        media_results['operations'] = {
            ...
            'improvement': f"{((query_time - optimized_query_time) / query_time * 100):.1f}%" if query_time > 0 else "N/A"
        }
        
        self.results['media'] = media_results
        
        self.stdout.write(f"    * Storage: {media_results['storage_backend'].split('.')[-1]}")
        self.stdout.write(f"    * Asset queries: {query_time:.1f}ms -> {optimized_query_time:.1f}ms")
    ...
    def generate_recommendations(self):
        ...
        slow_apis = [name for name, data in api_results.items() 
        ...
        slow_blocks = [str(count) for count, data in block_results.items() 
    ...
    def output_results(self):
        ...
        if 'page_tree' in db:
            ...
            self.stdout.write(f"  • Naive: {pt['naive']['queries']} queries, {pt['naive']['time']*1000:.1f}ms")
            self.stdout.write(f"  • Optimized: {pt['optimized']['queries']} queries, {pt['optimized']['time']*1000:.1f}ms")
            self.stdout.write(f"  • Improvement: {pt['improvement']}")
        
        if 'blog_posts' in db:
            bp = db['blog_posts']
            self.stdout.write("Blog Post Queries:")
            self.stdout.write(f"  • Naive: {bp['naive']['queries']} queries, {bp['naive']['time']*1000:.1f}ms")
            self.stdout.write(f"  • Optimized: {bp['optimized']['queries']} queries, {bp['optimized']['time']*1000:.1f}ms")
        ...
        for name, data in self.results['api'].items():
            status_icon = "[OK]" if data['status'] == 'good' else "[WARN]" if data['status'] == 'warning' else "[CRITICAL]"
            self.stdout.write(f"{status_icon} {name}: {data['avg_time_ms']}ms avg ({data['min_time_ms']}-{data['max_time_ms']}ms)")
        ...
        self.stdout.write(f"Redis Available: {'Yes' if cache['redis_available'] else 'No'}")
        ops = cache['operations']
        self.stdout.write(f"Operations: Set {ops['set_time_ms']}ms, Get {ops['get_time_ms']}ms, Delete {ops['delete_time_ms']}ms")
        ...
        for count, data in self.results['blocks'].items():
            status_icon = "[OK]" if data['status'] == 'good' else "[WARN]" if data['status'] == 'warning' else "[CRITICAL]"
            self.stdout.write(f"{status_icon} {count} blocks: {data['avg_time_ms']}ms total, {data['per_block_ms']}ms per block")
        ...
        self.stdout.write(f"Asset Queries: {ops['asset_query_ms']}ms → {ops['optimized_query_ms']}ms ({ops['improvement']} improvement)")
        
        # Recommendations
        self.stdout.write("\nRECOMMENDATIONS")
        self.stdout.write("-" * 17)
        for i, rec in enumerate(self.results['recommendations'], 1):
            severity_icon = "[HIGH]" if rec['severity'] == 'high' else "[MED]" if rec['severity'] == 'medium' else "[LOW]"
    ...
    def calculate_performance_score(self):
        ...
        if score >= 90:
            self.stdout.write(self.style.SUCCESS("   Excellent! Your CMS is highly optimized."))
        elif score >= 75:
            self.stdout.write(self.style.SUCCESS("   Good performance with room for minor improvements."))
        elif score >= 60:
            self.stdout.write(self.style.WARNING("   Decent performance, consider implementing recommendations."))
        else:
            self.stdout.write(self.style.ERROR("   Performance needs attention. Please review recommendations."))
