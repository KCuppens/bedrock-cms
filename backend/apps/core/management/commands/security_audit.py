...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(
                self.style.ERROR(f'Security audit failed: {e}')
            )
            return
    ...
    def _show_endpoint_details(self, report):
        ...
        for endpoint in report['endpoints']:
            ...
            if endpoint['permission_classes']:
                self.stdout.write(f'   Permissions: {", ".join(endpoint["permission_classes"])}')
            
            if endpoint['throttle_classes']:
                self.stdout.write(f'   Throttling: {", ".join(endpoint["throttle_classes"])}')
            
            if endpoint['authentication_classes']:
                self.stdout.write(f'   Authentication: {", ".join(endpoint["authentication_classes"])}')
