...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--clear-all',
            action='store_true',
            help='Clear all CMS cache entries'
        )
        ...
        parser.add_argument(
            '--test-keys',
            action='store_true',
            help='Test cache key generation'
        )
    ...
    def show_help(self):
        ...
        self.stdout.write('  python manage.py cache_manage --invalidate-content blog.blogpost')
    ...
    def clear_all_cache(self):
        ...
        self.stdout.write(
            self.style.SUCCESS(f'Successfully cleared all cache entries in {end_time - start_time:.2f}s')
        )
    ...
    def invalidate_pages(self):
        ...
        try:
            ...
            cache_manager.delete_pattern(f"{cache_manager.key_builder.prefix}:sm:*")  # Sitemaps
            
            self.stdout.write(
                self.style.SUCCESS('Successfully invalidated page cache')
            )
        except Exception as e:
            self.stdout.write(
                self.style.ERROR(f'Error invalidating page cache: {e}')
            )
    ...
    def _warm_page_cache(self):
        ...
        try:
            ...
            # Get published pages
            pages = Page.objects.filter(status='published').select_related('locale')[:20]
            
            for page in pages:
                if self.verbose:
                    self.stdout.write(f'  Warming page: {page.path} ({page.locale.code})')
    ...
    def _warm_blog_cache(self):
        ...
        try:
            ...
            posts = BlogPost.objects.filter(
                status='published'
            ).select_related('locale')[:10]
            
            for post in posts:
                if self.verbose:
                    self.stdout.write(f'  Warming blog post: {post.slug} ({post.locale.code})')
    ...
    def test_cache_keys(self):
        ...
        # Test content keys
        content_key = cache_manager.key_builder.content_key('blog.blogpost', 'en', 'my-post')
        ...
        # Test search keys
        search_key = cache_manager.key_builder.search_key('django cms', {'locale': 'en'})
