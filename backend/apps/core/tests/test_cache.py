...
from apps.core.signals import (
    invalidate_page_cache, invalidate_blog_cache, 
    invalidate_content_cache, invalidate_all_cache
)
...
class CacheKeyBuilderTests(TestCase):
    ...
    def test_search_key_with_filters(self):
        ...
        self.assertTrue(key.startswith('test:s:'))
        self.assertEqual(len(key.split(':')), 4)  # prefix:namespace:query_hash:filter_hash
...
class CacheManagerTests(TestCase):
    ...
    def test_get_or_set(self):
        ...
        result2 = self.cache_manager.get_or_set(key, lambda: {'different': True}, timeout=60)
