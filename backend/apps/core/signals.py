...
def invalidate_content_cache(instance, model_label):
    ...
    try:
        ...
        if config.locale_field:
            ...
            if locale_obj:
                locale_code = locale_obj.code if hasattr(locale_obj, 'code') else str(locale_obj)
...
def invalidate_cache_on_m2m_change(sender, instance, action, pk_set, **kwargs):
    ...
    if action in ['post_add', 'post_remove', 'post_clear']:
        try:
            ...
            logger.debug("Cache invalidated for M2M change on %s {instance.id}", model_label)
            
        except Exception as e:
            logger.warning("Error invalidating cache for M2M change on %s: {e}", instance)
...
def send_cdn_purge_webhook(keys: list, tags: list = None):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.warning("CDN purge webhook failed: %s {response.text}", response.status_code)
...
def invalidate_content_type_cache(model_label: str):
    ...
    try:
        ...
        config = content_registry.get_config(model_label)
        if not config:
            logger.warning("Unknown model label for cache invalidation: %s", model_label)
            return
...
def invalidate_blog_settings_cache(blog_settings):
    ...
    try:
        ...
        for post in posts:
            cache_manager.invalidate_blog_post(
                locale=post.locale.code,
                slug=post.slug
            )
        
        logger.info("Blog settings cache invalidated for locale %s", blog_settings.locale.code)
