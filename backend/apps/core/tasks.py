    ...
    try:
        ...
        if content_type == "blog_post":
            ...
            tracker, created = BlogPostViewTracker.objects.get_or_create(
                blog_post=post, defaults={"view_count": 0, "unique_view_count": 0}
            )
            ...
            PageView.objects.create(
                page=page, user_id=user_id, viewed_at=timezone.now()
            )
...
@shared_task(bind=True, max_retries=3)
def process_search_index_async(
    self, model_label, object_id, action="update"
):  # noqa: C901
    """
    ...
    try:
        ...
        if action == "update":
            ...
            get_search_service().index_object(model_label, object_id)

        elif action == "delete":

            get_search_service().remove_from_index(model_label, object_id)
...
def generate_thumbnails_async(image_id):  # noqa: C901
    ...
    try:
        ...
        file_obj = FileUpload.objects.get(id=image_id)

        # Define thumbnail sizes

        sizes = {"small": (150, 150), "medium": (300, 300), "large": (800, 800)}
...
def cleanup_old_revisions():  # noqa: C901
    ...
    try:
        ...
        pages_with_many_revisions = (
            PageRevision.objects.values("page")
        ...
        for page_data in pages_with_many_revisions:
            ...
            PageRevision.objects.filter(id__in=[r.id for r in old_revisions]).delete()

        # Clean up blog post revisions

        posts_with_many_revisions = (
            BlogPostRevision.objects.values("blog_post")
            .annotate(revision_count=Count("id"))
            .filter(revision_count__gt=50)
        )
