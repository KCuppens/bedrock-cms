...
class CacheWarmer:
    ...
    def warm_homepage_cache():
        ...
        for locale in Locale.objects.filter(is_active=True):
            ...
            for page in menu_pages:
                configs.append({
                    'key': cache_manager.key_builder.page_key(locale.code, page.path),
                    'callable': 'apps.cms.utils.get_page_data',
                    'args': [page.id],
    ...
    def warm_popular_content():
        ...
        featured_posts = BlogPost.objects.filter(
            status='published',
            featured=True
        ).select_related('locale').order_by('-published_at')[:20]
        ...
        recent_posts = BlogPost.objects.filter(
            status='published'
        ).select_related('locale').order_by('-published_at')[:30]
...
class Command(BaseCommand):
    ...
    def show_stats(self):
        ...
        for key in ['perf:slow:/api/v1/cms/pages/', 'perf:slow:/api/v1/blog/posts/']:
            data = cache.get(key)
            if data:
                self.stdout.write(f"  {key}: {data['avg_time']:.3f}s (n={data['count']})")