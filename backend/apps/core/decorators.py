...
def cache_response(timeout=300, key_prefix='', vary_on_headers=None, cache_errors=False):
    """
...
def conditional_cache(condition_func, timeout=300):
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            # Check condition
            should_cache = condition_func(*args, **kwargs) if callable(condition_func) else condition_func
            
            if not should_cache:
                return func(*args, **kwargs)
            
            # Generate cache key
            cache_key = _generate_cache_key(prefix=func.__name__, args=args, kwargs=kwargs)
