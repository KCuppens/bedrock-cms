...
class SecurityAuditReport:
    ...
    def _audit_endpoint_security(self, endpoint_info, view_class):
        ...
        if not auth_classes:
            ...
            if (
                ...
                and "DEFAULT_AUTHENTICATION_CLASSES" in settings.REST_FRAMEWORK
            ):

                auth_classes = [
                    cls.split(".")[-1]
                    for cls in settings.REST_FRAMEWORK["DEFAULT_AUTHENTICATION_CLASSES"]
                ]
        ...
        if not perm_classes:
            ...
            if (
                hasattr(settings, "REST_FRAMEWORK")
                and "DEFAULT_PERMISSION_CLASSES" in settings.REST_FRAMEWORK
            ):

                perm_classes = [
                    cls.split(".")[-1]
                    for cls in settings.REST_FRAMEWORK["DEFAULT_PERMISSION_CLASSES"]
                ]
        ...
        if not throttle_classes:
            ...
            if (
                hasattr(settings, "REST_FRAMEWORK")
                and "DEFAULT_THROTTLE_CLASSES" in settings.REST_FRAMEWORK
            ):

                throttle_classes = [
                    cls.split(".")[-1]
                    for cls in settings.REST_FRAMEWORK["DEFAULT_THROTTLE_CLASSES"]
                ]
        ...
        if self._is_admin_endpoint(endpoint_info["path"]):
            ...
            if (
                "IsAdminUser" not in perm_classes
                and "DjangoModelPermissions" not in perm_classes
            ):
        ...
        if "media" in endpoint_info["path"] and "POST" in endpoint_info.get(
            "methods", ["POST"]
        ):
        ...
        self.issues.extend(
            [{"endpoint": endpoint_info["path"], "issue": issue} for issue in issues]
        )
    ...
    def _generate_recommendations(self):
        ...
        unauth_count = len(
            [i for i in self.issues if "authentication" in i["issue"].lower()]
        )

        csrf_count = len([i for i in self.issues if "csrf" in i["issue"].lower()])

        throttle_count = len(
            [w for w in self.warnings if "throttling" in w["warning"].lower()]
        )
