...
from .models import (
    Locale, TranslationUnit, UiMessage, UiMessageTranslation,
    TranslationGlossary, TranslationQueue, TranslationHistory
)
from .serializers import (
    LocaleSerializer, TranslationUnitSerializer, TranslationUnitUpdateSerializer,
    UiMessageSerializer, UiMessageTranslationSerializer,
    ObjectTranslationStatusSerializer, BulkTranslationUpdateSerializer,
    TranslationGlossarySerializer, GlossarySearchSerializer,
    TranslationQueueSerializer, TranslationHistorySerializer,
    TranslationApprovalSerializer, MachineTranslationSuggestionSerializer,
    BulkUiMessageUpdateSerializer, NamespaceSerializer,
    TranslationAssignmentSerializer, BulkAssignmentSerializer
)
...
class TranslationUnitViewSet(viewsets.ModelViewSet):
    ...
    @extend_schema(
        ...
        parameters=[
            OpenApiParameter('model_label', str, description='Model label (e.g., cms.page)'),
            OpenApiParameter('object_id', int, description='Object ID'),
            OpenApiParameter('target_locale', str, description='Target locale code (optional)'),
        ]
    )
    ...
    def assign(self, request, pk=None):
        ...
        TranslationHistory.objects.create(
            ...
            previous_target_text=translation_unit.target_text,
            new_target_text=translation_unit.target_text,
            comment=f"{action.capitalize()}: {comment}" if comment else action.capitalize(),
            performed_by=request.user
        )
        ...
        for translation_unit in translation_units:
            try:
                ...
                TranslationHistory.objects.create(
                    ...
                    previous_target_text=translation_unit.target_text,
                    new_target_text=translation_unit.target_text,
                    comment=f"Bulk {action}: {comment}" if comment else f"Bulk {action}",
                    performed_by=request.user
                )
...
class UiMessageViewSet(viewsets.ModelViewSet):
    ...
    @extend_schema(
        ...
        parameters=[
            OpenApiParameter('direction', str, description='Direction: import, export, or sync'),
            OpenApiParameter('locale', str, description='Specific locale code (optional)'),
            OpenApiParameter('namespace', str, description='Namespace (default: django)'),
        ]
    )
    ...
    def sync_po_files(self, request):
        ...
        try:
            ...
            command_args = ['sync_po_files', f'--direction={direction}', f'--namespace={namespace}']
            if locale_code:
    ...
    @action(detail=False, methods=['get'], url_path='bundle/(?P<locale_code>[^/.]+)', 
        ...
        request={
            'application/json': {
                ...
                'properties': {
                    ...
                    'source': {'type': 'string', 'description': 'Source of keys (build, runtime-discovery)'}
                }
            }
        }
    )
    ...
    def sync_keys(self, request):
        ...
        source = request.data.get('source', 'unknown')  # 'build', 'runtime-discovery', etc.
        
        # Determine if we should auto-approve
        auto_approve = source == 'build' or getattr(settings, 'AUTO_APPROVE_TRANSLATIONS', False)
        ...
        for key_data in keys_data:
            ...
            try:
                ...
                if was_created:
                    ...
                    if (key_data.get('defaultValue') and 
                        key_data['defaultValue'] != key and 
                        ui_message.default_value == key):
    ...
    @action(detail=False, methods=['get'], url_path='discovery-stats',
            permission_classes=[permissions.AllowAny])
    def discovery_stats(self, request):
        ...
        by_namespace = UiMessage.objects.values('namespace').annotate(
            count=Count('id')
        ).order_by('-count')
        ...
        for locale in Locale.objects.filter(is_active=True):
            ...
            untranslated_count = UiMessage.objects.exclude(
                id__in=translated
            ).count()
    ...
    def import_json(self, request):
        ...
        if not locale_code:
            return Response({
                'status': 'error',
                'message': 'Locale code is required'
            }, status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            for key, value in translations.items():
                try:
                    ...
                    try:
                        ...
                        if message.namespace != namespace or message.default_value != value:
                    ...
                    translation, translation_created = UiMessageTranslation.objects.get_or_create(
                        message=message,
                        locale=locale,
                        defaults={
                            'value': value,
            ...
            total_changes = created_messages + updated_messages + created_translations + updated_translations
            ...
            if total_changes == 0 and errors:
                ...
                response_data = {
                    ...
                    'details': {
                        ...
                        'errors': errors[:10] if len(errors) > 10 else errors  # Show first 10 errors
                    }
                }
                ...
                response_data = {
                    ...
                    'details': {
                        ...
                        'translations_created': created_translations,
                        'translations_updated': updated_translations,
                        'errors': errors[:5] if len(errors) > 5 else errors  # Show first 5 errors
                    }
                }
            else:
                # Full success
...
class UiMessageTranslationViewSet(viewsets.ModelViewSet):
    ...
    queryset = UiMessageTranslation.objects.select_related('message', 'locale', 'updated_by')
    ...
    def namespaces(self, request):
        ...
        namespaces = UiMessage.objects.values('namespace').annotate(
        ...
        for ns in namespaces:
            ...
            for locale in locales:
                ...
                locale_stats.append({
                    ...
                    'total': message_count,
                    'percentage': (translated_count / message_count * 100) if message_count > 0 else 0
                })
    ...
    @extend_schema(
        ...
        parameters=[
            OpenApiParameter('locale', str, description='Target locale code (required)', required=True),
            OpenApiParameter('source_locale', str, description='Source locale code (default: en)', required=False),
            OpenApiParameter('namespace', str, description='Filter by namespace (optional)', required=False),
            OpenApiParameter('max_translations', int, description='Maximum number of translations to create (optional)', required=False),
        ]
    )
    @action(detail=False, methods=['post'])
    def bulk_auto_translate(self, request):
        ...
        if not locale_code:
            return Response({
                'status': 'error',
                'message': 'Target locale is required'
            }, status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            if is_eager:
                ...
                try:
                    ...
                    return Response({
                        'status': 'success',
                        'message': result.get('message', 'Auto-translation completed'),
                        'task_id': None,
    ...
    @extend_schema(
        ...
        parameters=[
            OpenApiParameter('task_id', str, description='Task ID to check', required=True),
        ]
    )
    @action(detail=False, methods=['get'])
        ...
        try:
            ...
            if task_result.status == 'PENDING':
                ...
                response_data.update({
                    ...
                    'results': result.get('details', {}),
                    'progress': {
                        'current': result.get('details', {}).get('total_processed', 0),
                        'total': result.get('details', {}).get('total_processed', 0),
                        'percentage': 100
                    }
                })
...
class TranslationGlossaryViewSet(viewsets.ModelViewSet):
    ...
    def search(self, request):
        ...
        queryset = queryset.filter(
            models.Q(term__icontains=term) | 
            models.Q(translation__icontains=term)
        )
...
class TranslationQueueViewSet(viewsets.ModelViewSet):
    ...
    queryset = TranslationQueue.objects.select_related(
        'translation_unit', 'assigned_to', 'created_by',
        'translation_unit__source_locale', 'translation_unit__target_locale'
    )
    ...
    def summary(self, request):
        ...
        for locale in locales:
            ...
            # Count by status
            status_counts = locale_queue.values('status').annotate(
                count=Count('id')
            ).order_by('status')
            ...
            # Count by priority
            priority_counts = locale_queue.values('priority').annotate(
                count=Count('id')
            ).order_by('priority')
        ...
        return Response({
            'overall': {
                ...
                'completion_percentage': round((overall_completed / overall_total * 100) if overall_total > 0 else 100, 1)
            },
