...
class Command(BaseCommand):
    ...
    DJANGO_STRINGS = {
        ...
        'validation.min_length': _('Ensure this value has at least %(limit_value)d character (it has %(show_value)d).'),
        'validation.max_length': _('Ensure this value has at most %(limit_value)d character (it has %(show_value)d).'),
        'validation.min_value': _('Ensure this value is greater than or equal to %(limit_value)s.'),
        'validation.max_value': _('Ensure this value is less than or equal to %(limit_value)s.'),
        ...
        'auth.password_reset_sent': _("We've emailed you instructions for setting your password."),
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            '--locale',
            type=str,
            help='Specific locale to import translations for'
        )
        parser.add_argument(
            '--namespace',
            type=str,
    ...
    def handle(self, *args, **options):
        ...
        for key, lazy_string in self.DJANGO_STRINGS.items():
            ...
            for locale in locales:
                ...
                if update_existing:
                    translation_obj, trans_created = UiMessageTranslation.objects.update_or_create(
                        message=ui_message,
                        locale=locale,
                        defaults={
                    ...
                    translation_obj, trans_created = UiMessageTranslation.objects.get_or_create(
                        message=ui_message,
                        locale=locale,
                        defaults={
                            'value': translated_value,
        ...
        for locale in locales:
            ...
            percentage = (translated / total_messages * 100) if total_messages > 0 else 0
            self.stdout.write(
