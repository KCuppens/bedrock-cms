...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        translated_messages = UiMessage.objects.filter(
            id__in=UiMessageTranslation.objects.filter(locale=locale).values_list('message_id', flat=True)
        ).count()
        self.stdout.write(f'Messages with {locale.code} translations: {translated_messages}')

        # Messages WITHOUT translations for this locale
        untranslated_messages = UiMessage.objects.exclude(
            id__in=UiMessageTranslation.objects.filter(locale=locale).values_list('message_id', flat=True)
        ).count()
        self.stdout.write(f'Messages WITHOUT {locale.code} translations: {untranslated_messages}')
        ...
        namespaces = UiMessage.objects.values('namespace').distinct().order_by('namespace')
        ...
        for ns in namespaces:
            ...
            self.stdout.write(f'  {namespace}: {ns_translated}/{ns_total} ({ns_percentage:.1f}%)')

        # Show recent translations
        self.stdout.write('\nRecent translations:')
        recent_translations = UiMessageTranslation.objects.filter(
            locale=locale
