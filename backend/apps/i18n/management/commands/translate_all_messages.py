...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            type=str,
            default='fr',
            help='Target locale code (default: fr)'
        )
        parser.add_argument(
            '--source',
            type=str,
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f'Translating ALL UI messages from {source_locale.name} to {target_locale.name}')
            ...
            for message in batch:
                try:
                    ...
                    if not message.default_value or not message.default_value.strip():
                        self.stdout.write(f'  [SKIP] {message.key}: empty default value')
                        skipped_count += 1
                        continue
                    ...
                    self.stdout.write(f'  -> Translating: "{message.default_value[:50]}{"..." if len(message.default_value) > 50 else ""}"')
                    ...
                    if translated_text and translated_text.strip():
                        if existing_translation:
                            ...
                            self.stdout.write(f'    [OK] Updated: "{translated_text[:50]}{"..." if len(translated_text) > 50 else ""}"')
                            updated_count += 1
                        else:
                            ...
                            self.stdout.write(f'    [OK] Created: "{translated_text[:50]}{"..." if len(translated_text) > 50 else ""}"')
                            translated_count += 1
                    else:
        ...
        total_translations = UiMessageTranslation.objects.filter(locale=target_locale).count()
        total_ui_messages = UiMessage.objects.count()
        coverage = (total_translations / total_ui_messages * 100) if total_ui_messages > 0 else 0
        
        self.stdout.write(f'\nFinal translation coverage for {target_locale.name}:')
        self.stdout.write(f'{total_translations}/{total_ui_messages} messages ({coverage:.1f}%)')