...
class Command(BaseCommand):
    ...
    def clear_cache(self, json_output=False):
        ...
        if json_output:
            ...
            self.stdout.write(
                self.style.SUCCESS('✓ Locale settings cache cleared')
            )
    ...
    def validate_consistency(self, json_output=False, fix_inconsistencies=False):
        ...
        if json_output:
            ...
            if validation_result['is_consistent']:
                self.stdout.write(
                    self.style.SUCCESS('✓ Database locales and Django settings are consistent')
                )
            else:
                self.stdout.write(
                    self.style.WARNING('⚠ Inconsistencies detected between database and Django settings:')
                )
                ...
                if fix_inconsistencies:
                    self.stdout.write('\n' + '='*50)
                    self.stdout.write('Fixing inconsistencies by syncing database to Django settings...')
                    self.sync_db_to_django(json_output=False)
        
        return validation_result

    def sync_db_to_django(self, json_output=False, update_settings_file=False, settings_path=None):
        ...
        if update_settings_file:
            ...
            if success:
                result['message'] = 'Database locales synced to Django settings and settings file updated'
            else:
                result['message'] = 'Database locales synced to Django cache, but settings file update failed'
                result['warning'] = 'Settings file was not updated. Changes are only in cache until server restart.'
        else:
            result['message'] = 'Database locales synced to Django settings cache'
            result['note'] = 'Changes are cached and will persist until server restart. Use --update-settings-file for permanent changes.'
        ...
        if json_output:
            ...
            if update_settings_file:
                if result['file_updated']:
                    ...
                    self.stdout.write(
                        self.style.WARNING('⚠ Settings file update failed - changes are only cached')
                    )
            else:
                self.stdout.write(
                    self.style.WARNING('ⓘ Changes are cached. Use --update-settings-file for permanent changes.')
                )
    ...
    def sync_django_to_db(self, json_output=False, default_locale_code=None):
        ...
        if django_languages:
            ...
            if created:
                created_count += 1
                result['created_locales'].append({'code': django_language_code, 'name': django_language_code.title()})
            else:
                updated_count += 1
                result['updated_locales'].append({'code': django_language_code, 'name': django_language_code.title()})
        ...
        if default_locale_code:
            try:
                ...
                raise CommandError(f'Locale with code "{default_locale_code}" not found')
        ...
        if json_output:
            ...
            self.stdout.write(
                self.style.SUCCESS(f'✓ Synced Django settings to database: {result["summary"]}')
            )
