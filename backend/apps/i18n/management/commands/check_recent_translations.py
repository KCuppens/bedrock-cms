...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f'Translations for {locale.name} ({locale.code}) created in the last {hours} hour(s):')
        ...
        if total_count == 0:
            ...
            total_translations = UiMessageTranslation.objects.filter(locale=locale).count()
            self.stdout.write(f'\nTotal translations for {locale.code}: {total_translations}')
            
            # Show the most recent translation
            latest = UiMessageTranslation.objects.filter(locale=locale).order_by('-created_at').first()
            if latest:
                self.stdout.write(f'Most recent translation was created at: {latest.created_at}')
