...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            'locale_code',
            type=str,
            help='Locale code to deactivate (e.g., es, fr, de)'
        )
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if locale.is_default:
                ...
                locale.save()
                self.stdout.write(
                    self.style.SUCCESS(f'Successfully deactivated locale: {locale_code}')
                )
        except Locale.DoesNotExist:
            self.stdout.write(
                self.style.ERROR(f'Locale {locale_code} does not exist')
            )