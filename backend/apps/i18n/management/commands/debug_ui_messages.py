...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            type=str,
            default='es',
            help='Target locale code (default: es)'
        )
        parser.add_argument(
            '--limit',
            type=int,
    ...
    def handle(self, *args, **options):
        ...
        empty_default_count = messages_query.filter(
            default_value__isnull=True
        ).count() + messages_query.filter(
            default_value=''
        ).count()
        ...
        non_empty_messages = messages_query.exclude(
            default_value__isnull=True
        ).exclude(default_value='')[:5]
