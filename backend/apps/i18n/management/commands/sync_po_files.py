...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--app',
            type=str,
            help='Specific Django app to sync translations for'
        )
        parser.add_argument(
            '--create-missing',
            action='store_true',
        ...
        for app_config in app_configs:
            ...
            for locale in locales:
                ...
                for entry in po:
                    ...
                    translation, trans_created = UiMessageTranslation.objects.update_or_create(
                        message=ui_message,
                        locale=locale,
                        defaults={
                            'value': entry.msgstr,
                            'status': 'approved' if not entry.fuzzy else 'needs_review'
                        }
                    )
    ...
    def export_to_po_files(self, locales, app_configs, namespace, create_missing):
        ...
        for app_config in app_configs:
            ...
            for locale in locales:
                ...
                for ui_message in messages_by_app[app_label]:
                    ...
                    msgid = ui_message.key.split('.', 1)[1] if '.' in ui_message.key else ui_message.key
        ...
        self.stdout.write(
            self.style.SUCCESS(f'Export complete: {exported_count} translations exported')
        )