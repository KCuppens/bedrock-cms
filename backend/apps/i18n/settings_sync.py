...
class DjangoSettingsSync:
    ...
    def get_default_language(cls) -> str:
        ...
        try:
            ...
            default_locale = Locale.objects.filter(is_default=True, is_active=True).first()
    ...
    def get_locale_settings(cls) -> Dict[str, Any]:
        ...
        try:
            ...
            active_locales = Locale.objects.filter(is_active=True).order_by('sort_order')
            default_locale = active_locales.filter(is_default=True).first()
            
            settings_dict = {
                'LANGUAGE_CODE': default_locale.code if default_locale else 'en',
                'LANGUAGES': [(locale.code, locale.name) for locale in active_locales],
                'RTL_LANGUAGES': [locale.code for locale in active_locales if locale.rtl],
    ...
    def clear_cache(cls):
        ...
        try:
            ...
            logger.warning(f"Failed to clear cache (cache backend may not be available): {e}")
    ...
    def update_settings_file(cls, settings_path: Optional[str] = None) -> bool:
        ...
        try:
            if not settings_path:
                ...
                possible_paths = [
                    Path(settings.BASE_DIR) / 'apps' / 'config' / 'settings' / settings_file,
            ...
            if 'LANGUAGES' in content and 'LANGUAGE_CODE' not in content.split('LANGUAGES')[0].split('\n')[-1]:
                import re
                content = re.sub(
                    r'LANGUAGES\s*=\s*\[.*?\]',
        ...
        try:
            with connection.cursor() as cursor:
                cursor.execute(
                    ...
                    else "SELECT 1 FROM information_schema.tables WHERE table_name='i18n_locale'"
