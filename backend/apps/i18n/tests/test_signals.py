...
class PageSignalsTest(TestCase):
    ...
    def test_create_page_translation_units_skip_flag(self):
        ...
        with patch('apps.i18n.signals.TranslationManager.create_translation_units') as mock_create_units:
            create_page_translation_units(
                sender=Page, 
                instance=page, 
                created=True
    ...
    def test_store_old_page_data_existing_page(self):
        ...
        self.assertEqual(page._old_blocks, [{'type': 'text', 'value': 'Original content'}])
...
class GenericSignalsTest(TestCase):
    ...
    def test_register_model_for_translation(self):
        ...
        with patch('apps.i18n.signals.ContentType.objects.get_for_model') as mock_get_ct, \
             patch('apps.i18n.signals.TranslationManager.register_translatable_fields') as mock_register, \
    ...
    def test_register_model_for_translation_without_fields(self):
        ...
        with patch('apps.i18n.signals.TranslationManager.register_translatable_fields') as mock_register, \
             patch('apps.i18n.signals.post_save.connect') as mock_connect:
...
class SignalIntegrationTest(TestCase):
    ...
    def test_page_save_creates_translation_units(self):
        ...
        expected_count = Locale.objects.filter(is_active=True).exclude(id=self.locale_en.id).count() * 2  # 2 fields
