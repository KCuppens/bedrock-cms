...
class ImportDjangoTranslationsCommandTest(TestCase):
    ...
    def test_import_django_strings(self):
        ...
        self.assertTrue(
            UiMessage.objects.filter(key='admin.save').exists()
        )
        self.assertTrue(
            UiMessage.objects.filter(key='actions.cancel').exists()
        )
    ...
    def test_import_django_strings_with_namespace(self):
        ...
        self.assertTrue(
            UiMessage.objects.filter(namespace='custom_django').exists()
        )
...
class SyncPoFilesCommandTest(TestCase):
    ...
    def test_export_to_po_files(self, mock_polib):
        ...
        with patch('apps.i18n.management.commands.sync_po_files.apps') as mock_apps:
            ...
            # Run command
            out = StringIO()
            call_command('sync_po_files', direction='export', create_missing=True, stdout=out)
    ...
    def test_sync_bidirectional(self):
        ...
        with patch('apps.i18n.management.commands.sync_po_files.Command.import_from_po_files') as mock_import, \
             patch('apps.i18n.management.commands.sync_po_files.Command.export_to_po_files') as mock_export:
