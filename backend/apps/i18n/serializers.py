...
class LocaleSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Locale
        fields = [
            'id', 'code', 'name', 'native_name', 'fallback', 'fallback_code', 
            'fallback_name', 'rtl', 'sort_order', 'is_active', 'is_default',
...
class TranslationUnitSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'model_label', 'object_id', 'field', 'source_locale', 
            'source_locale_code', 'target_locale', 'target_locale_code',
            'source_text', 'target_text', 'status', 'updated_by', 'updated_by_email',
            'is_complete', 'created_at', 'updated_at'
        ]
        read_only_fields = [
            'id', 'model_label', 'object_id', 'field', 'source_locale', 
            'source_locale_code', 'source_text', 'created_at'
        ]
...
class UiMessageSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = UiMessage
        fields = [
            'id', 'key', 'namespace', 'description', 'default_value',
            'translation_count', 'created_at', 'updated_at'
        ]
        read_only_fields = ['id', 'created_at', 'updated_at']
...
class UiMessageTranslationSerializer(serializers.ModelSerializer):
    ...
    message_namespace = serializers.CharField(source='message.namespace', read_only=True)
    ...
    class Meta:
        ...
        fields = [
            'id', 'message', 'message_key', 'message_namespace', 
            'locale', 'locale_code', 'value', 'status', 
            'updated_by', 'updated_by_email', 'created_at', 'updated_at'
        ]
        read_only_fields = [
            'id', 'message_key', 'message_namespace', 'locale_code',
...
class BulkTranslationUpdateSerializer(serializers.Serializer):
    ...
    units = serializers.ListField(
        ...
        help_text="List of translation unit updates"
    )
...
class TranslationGlossarySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'term', 'source_locale', 'source_locale_code', 
            'target_locale', 'target_locale_code', 'translation', 
            'context', 'category', 'is_verified', 'created_by', 
            'created_by_email', 'updated_by', 'updated_by_email',
            'created_at', 'updated_at'
        ]
        read_only_fields = [
            'id', 'source_locale_code', 'target_locale_code',
...
class GlossarySearchSerializer(serializers.Serializer):
    ...
    source_locale = serializers.CharField(required=False, help_text="Source locale code")
    target_locale = serializers.CharField(required=False, help_text="Target locale code")
    category = serializers.CharField(required=False, help_text="Category to filter by")
    verified_only = serializers.BooleanField(default=False, help_text="Only return verified terms")


class TranslationQueueSerializer(serializers.ModelSerializer):
    """Serializer for TranslationQueue model."""
    
    translation_unit_id = serializers.IntegerField(source='translation_unit.id', read_only=True)
    source_text = serializers.CharField(source='translation_unit.source_text', read_only=True)
    target_text = serializers.CharField(source='translation_unit.target_text', read_only=True)
    source_locale_code = serializers.CharField(source='translation_unit.source_locale.code', read_only=True)
    target_locale_code = serializers.CharField(source='translation_unit.target_locale.code', read_only=True)
    model_label = serializers.CharField(source='translation_unit.model_label', read_only=True)
    ...
    class Meta:
        ...
        fields = [
            'id', 'translation_unit', 'translation_unit_id', 'source_text', 
            'target_text', 'source_locale_code', 'target_locale_code',
            'model_label', 'field_name', 'status', 'priority',
            'assigned_to', 'assigned_to_email', 'deadline', 'notes',
            'machine_translation_suggestion', 'mt_service', 'word_count',
            'estimated_hours', 'created_by', 'created_by_email', 
            'is_overdue', 'created_at', 'updated_at'
        ]
        read_only_fields = [
            'id', 'translation_unit_id', 'source_text', 'target_text',
            'source_locale_code', 'target_locale_code', 'model_label',
            'field_name', 'assigned_to_email', 'created_by_email',
            'is_overdue', 'word_count', 'created_at', 'updated_at'
        ]
...
class TranslationHistorySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'translation_unit', 'translation_unit_id', 'action',
            'previous_status', 'new_status', 'previous_target_text',
            'new_target_text', 'comment', 'performed_by', 
            'performed_by_email', 'created_at'
        ]
        read_only_fields = [
            'id', 'translation_unit_id', 'performed_by_email', 'created_at'
        ]
...
class TranslationApprovalSerializer(serializers.Serializer):
    ...
    comment = serializers.CharField(required=False, help_text="Optional comment for the action")
...
class MachineTranslationSuggestionSerializer(serializers.Serializer):
    ...
    source_locale = serializers.CharField(
        max_length=10,
        required=True,
        help_text="Source locale code"
    )
    target_locale = serializers.CharField(
        max_length=10,
        required=True,
        help_text="Target locale code for translation"
    )
...
class BulkUiMessageUpdateSerializer(serializers.Serializer):
    ...
    updates = serializers.ListField(
        child=serializers.DictField(),
        help_text="List of message translation updates"
    )
...
class BulkAssignmentSerializer(serializers.Serializer):
    ...
    def validate_translation_unit_ids(self, value):
        ...
        if not value:
            raise serializers.ValidationError("At least one translation unit ID is required")
        
        # Check if all IDs exist
        existing_ids = set(TranslationUnit.objects.filter(id__in=value).values_list('id', flat=True))
