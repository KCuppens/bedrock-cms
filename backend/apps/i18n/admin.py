...
class LocaleAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'code', 'name', 'native_name', 'is_default_display', 
        'is_active', 'rtl_display', 'fallback', 'sort_order'
    ]
    ...
    fieldsets = [
        ...
        ('Configuration', {
            'fields': ['fallback', 'rtl', 'sort_order']
        }),
        ('Status', {
    ...
    def make_default(self, request, queryset):
        ...
        if queryset.count() != 1:
            self.message_user(request, "Please select exactly one locale to make default.", level='error')
            return
...
class TranslationGlossaryAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'term', 'translation', 'source_locale', 'target_locale',
    ...
    fieldsets = [
        ('Translation', {
            'fields': ['term', 'translation', 'context']
        }),
        ('Locales', {
...
class TranslationQueueAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'translation_unit', 'status', 'priority', 'assigned_to',
        'deadline', 'word_count', 'is_overdue', 'created_at'
    ]
    list_filter = ['status', 'priority', 'assigned_to', 'deadline']
    search_fields = ['translation_unit__source_text', 'translation_unit__target_text', 'notes']
    ordering = ['-priority', 'deadline']
    readonly_fields = ['created_at', 'updated_at', 'word_count', 'is_overdue']
    
    fieldsets = [
        ('Translation', {
            'fields': ['translation_unit', 'status', 'priority']
        }),
        ('Assignment', {
            'fields': ['assigned_to', 'deadline', 'estimated_hours']
        }),
        ('Machine Translation', {
        ...
        ('Metadata', {
            'fields': ['word_count', 'is_overdue', 'created_by', 'created_at', 'updated_at'],
            'classes': ['collapse']
        })
    ]
...
class TranslationHistoryAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'translation_unit', 'action', 'performed_by', 'created_at'
    ]
...
class TranslationUnitAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'content_type', 'content_object', 'field', 'source_locale', 'target_locale',
...
class UiMessageTranslationAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'message', 'locale', 'status', 'updated_by', 'updated_at'
    ]
