...
class DeepLTranslationService:
    ...
    def __init__(self):
        ...
        if not self.api_key:
            logger.warning("DEEPL_API_KEY not configured - translations will use fallback")
        else:
    ...
    def translate(self, text, source_lang, target_lang):
        ...
        logger.info(f"DeepL translate request: '{text}' from {source_lang}({source_code}) to {target_lang}({target_code})")
        
        try:
            logger.info(f"Making DeepL API request to: {self.base_url}/translate")
            logger.info(f"Request data: text='{text}', source_lang={source_code}, target_lang={target_code}")
            ...
            if response.status_code == 200:
                ...
                if result.get("translations"):
                    ...
                    logger.info(f"DeepL translation successful: '{text}' -> '{translated_text}' ({source_lang}->{target_lang})")
        ...
        logger.warning(f"DeepL translation returned None for: '{text}' ({source_lang}->{target_lang})")
        return None
    ...
    def _fallback_translate(self, text, source_lang, target_lang):
        ...
        logger.info(f"No fallback translation for '{text}' from {source_lang} to {target_lang}")
