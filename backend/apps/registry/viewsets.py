...
from .serializers import get_serializer_for_config, RegistrySerializer, RegistrySummarySerializer
        ...
        if config.slug_field:
            @extend_schema(
                ...
                parameters=[
                    ...
                    OpenApiParameter('locale', str, description='Locale code (optional)'),
                ]
            )
...
class RegistryViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def list(self, request):
        ...
        serializer = RegistrySerializer([config.to_dict() for config in configs], many=True)
        return Response(serializer.data)
