...
class ContentSerializerFactory:
    ...
    @classmethod
    def create_serializer(cls, config: ContentConfig) -> Type[serializers.ModelSerializer]:
        """
    ...
    def _get_custom_fields(cls, config: ContentConfig) -> Dict[str, Any]:
        ...
        if config.locale_field:
            custom_fields['locale_code'] = serializers.CharField(
                source=f'{config.locale_field}.code', 
                read_only=True
            )
            custom_fields['locale_name'] = serializers.CharField(
                source=f'{config.locale_field}.name', 
                read_only=True
            )
...
class RegistrySerializer(serializers.Serializer):
    ...
    form_fields = serializers.ListField(
        child=serializers.CharField(), 
        allow_null=True
    )
...
def get_serializer_for_config(config: ContentConfig) -> Type[serializers.ModelSerializer]:
    """
