...
@extend_schema_view(
    list=extend_schema(
        ...
        parameters=[
            OpenApiParameter('action', type=str, description='Action type to filter by'),
            OpenApiParameter('user', type=int, description='User ID to filter by'),
            OpenApiParameter('date_from', type=str, description='Start date (YYYY-MM-DD)'),
            OpenApiParameter('date_to', type=str, description='End date (YYYY-MM-DD)'),
        ]
...
@extend_schema_view(
    list=extend_schema(
        ...
        parameters=[
            OpenApiParameter('date_from', type=str, description='Start date (YYYY-MM-DD)'),
            OpenApiParameter('date_to', type=str, description='End date (YYYY-MM-DD)'),
            OpenApiParameter('content_category', type=str, description='Content category to filter by'),
        ]
    )
)
...
@extend_schema_view(
    list=extend_schema(
        ...
        parameters=[
            OpenApiParameter('assessment_type', type=str, description='Assessment type to filter by'),
            OpenApiParameter('status', type=str, description='Status to filter by'),
            OpenApiParameter('assigned_to', type=int, description='Assigned user ID to filter by'),
        ]
    ),
    create=extend_schema(
...
@extend_schema_view(
    list=extend_schema(
        ...
        parameters=[
            OpenApiParameter('category', type=str, description='Risk category to filter by'),
            OpenApiParameter('status', type=str, description='Status to filter by'),
            OpenApiParameter('severity', type=str, description='Severity level to filter by'),
        ]
    )
)
...
@extend_schema_view(
    list=extend_schema(
        ...
        parameters=[
            OpenApiParameter('threat_type', type=str, description='Threat type to filter by'),
            OpenApiParameter('status', type=str, description='Status to filter by'),
            OpenApiParameter('severity', type=str, description='Severity level to filter by'),
        ]
    ),
    create=extend_schema(
        summary="Create threat record",
...
class AnalyticsAPIViewSet(viewsets.GenericViewSet):
    ...
    @extend_schema(
        ...
        parameters=[
            OpenApiParameter('days', type=int, description='Number of days to include (default: 30)'),
            OpenApiParameter('period', type=str, description='Grouping period: daily, weekly, monthly'),
        ],
        ...
        for item in queryset:
            ...
            bounced_sessions = PageView.objects.filter(
                ...
                session_id__in=PageView.objects.filter(
                    ...
                    page_count=Count('id')
                ).filter(page_count=1).values_list('session_id', flat=True)
            ).count()
    ...
    @extend_schema(
        ...
        parameters=[
            OpenApiParameter('days', type=int, description='Number of days to include (default: 30)'),
            OpenApiParameter('limit', type=int, description='Limit results (default: 20)'),
        ],
    ...
    def page_views_analytics(self, request):
        ...
        top_pages = PageView.objects.filter(
            viewed_at__date__range=[start_date, end_date],
            page__isnull=False
        ).values('page__title', 'url').annotate(
            views=Count('id'),
    ...
    def dashboard_summary(self, request):
        ...
        ).aggregate(avg_load_time=Avg('load_time'))['avg_load_time'] or 0
    ...
    def risk_timeline(self, request):
        ...
        for i in range(days + 1):
            ...
            risks_identified = Risk.objects.filter(
                identified_at__date=date
            ).count()
            
            risks_mitigated = Risk.objects.filter(
                status='mitigated',
                last_reviewed__date=date
            ).count()
            
            avg_risk_score = Risk.objects.filter(
                identified_at__date__lte=date,
                status__in=['identified', 'assessed']
    ...
    def threat_statistics(self, request):
        ...
        threat_stats = Threat.objects.filter(
            detected_at__date__range=[start_date, end_date]
        ).values('threat_type').annotate(
            count=Count('id'),
            severity_breakdown=Count('severity')
        ).order_by('-count')
        
        data = []
        for stat in threat_stats:
            # Get severity breakdown for this threat type
            severity_breakdown = Threat.objects.filter(
                threat_type=stat['threat_type'],
                detected_at__date__range=[start_date, end_date]
            ).values('severity').annotate(count=Count('id'))
            
            severity_dict = {item['severity']: item['count'] for item in severity_breakdown}
    ...
    @extend_schema(
        ...
        parameters=[
            OpenApiParameter('format', type=str, description='Export format: csv, json'),
            OpenApiParameter('type', type=str, description='Data type: traffic, threats, risks, assessments'),
