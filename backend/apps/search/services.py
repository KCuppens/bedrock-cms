...
from .models import SearchIndex, SearchQuery as SearchQueryLog, SearchSuggestion
...
class SearchService:
    ...
    def search(
        ...
        return {
            'results': [self._serialize_search_result(result) for result in page_obj.object_list],
            'pagination': {
                'page': page,
                'page_size': page_size,
    ...
    def get_suggestions(self, query: str, limit: int = 10) -> List[str]:
        ...
        suggestions = self.suggestion_model.objects.filter(
            normalized_text__startswith=normalized_query,
            is_active=True
    ...
    def get_search_analytics(self, days: int = 30) -> Dict[str, Any]:
        ...
        queries = self.query_log_model.objects.filter(
            created_at__gte=start_date
        )
        
        # Basic stats
        total_queries = queries.count()
        avg_results = queries.aggregate(avg_results=Avg('result_count'))['avg_results'] or 0
        ...
        # Zero result queries
        zero_result_queries = queries.filter(result_count=0).values(
            'query_text'
        ).annotate(
            count=Count('query_text')
        ).order_by('-count')[:10]
