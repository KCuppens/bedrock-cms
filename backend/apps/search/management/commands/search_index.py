...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--model',
            type=str,
            help='Specific model to reindex (e.g., blog.blogpost)'
        )
        
        parser.add_argument(
            '--clear',
            action='store_true',
            help='Clear all search index entries'
        )
    ...
    def show_stats(self):
        ...
        content_types = SearchIndex.objects.values(
        ...
        for ct in content_types:
            ...
            self.stdout.write(f'  {app_label}.{model}: {total} total, {published} published')
        
        self.stdout.write('')
        
        # By category
        self.stdout.write('By search category:')
        categories = SearchIndex.objects.values('search_category').annotate(
            total=Count('id'),
            published=Count('id', filter=models.Q(is_published=True))
        ).order_by('-total')
    ...
    def reindex_all(self, batch_size):
        ...
        self.stdout.write(self.style.SUCCESS('Reindexing all registered content types...'))
    ...
    def reindex_model(self, model_label, batch_size):
        ...
        self.stdout.write(
            self.style.SUCCESS(f'Reindexing model: {model_label}')
        )
        
        # Get model configuration
        config = content_registry.get_config(model_label)
        if not config:
            raise CommandError(f'Model {model_label} is not registered with content registry')
            ...
            for obj in batch:
                try:
                    ...
                    if self.verbose and indexed_count % 50 == 0:
                        self.stdout.write(f'    Indexed {indexed_count}/{total_objects}...')
