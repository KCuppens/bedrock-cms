...
class SearchIndexAdmin(admin.ModelAdmin):
    ...
    list_display = (
        'title', 'content_type_display', 'search_category', 
        'is_published', 'search_weight', 'locale_code', 'indexed_at'
    )
    list_filter = (
        'is_published', 'search_category', 'content_type', 
        'locale_code', 'indexed_at'
    )
    ...
    fieldsets = (
        ...
        ('Search Data', {
            'fields': ('title', 'content', 'excerpt', 'search_category', 'search_tags')
        }),
        ('Metadata', {
            'fields': (
                'url', 'image_url', 'locale_code', 
                'is_published', 'published_at', 'search_weight'
            )
        }),
        ('Timestamps', {
            'fields': ('indexed_at', 'created_at'),
            'classes': ('collapse',)
        }),
    )
...
class SearchQueryAdmin(admin.ModelAdmin):
    ...
    list_display = (
        'query_text', 'user_display', 'result_count',
        'execution_time_ms', 'clicked_result', 'created_at'
    )
    list_filter = ('result_count', 'created_at', 'user')
    search_fields = ('query_text', 'user__email', 'ip_address')
    readonly_fields = (
        'id', 'query_text', 'filters', 'user', 'session_key',
        'ip_address', 'result_count', 'execution_time_ms',
        'clicked_result', 'click_position', 'created_at'
    )
...
class SearchSuggestionAdmin(admin.ModelAdmin):
    ...
    list_display = (
        'suggestion_text', 'search_count', 'result_count',
        'click_through_rate', 'is_promoted', 'is_active', 'last_searched_at'
    )
    list_filter = ('is_active', 'is_promoted', 'last_searched_at')
    search_fields = ('suggestion_text', 'normalized_text')
    readonly_fields = (
        'id', 'normalized_text', 'search_count', 'result_count',
        'click_through_rate', 'last_searched_at', 'created_at', 'updated_at'
    )
    ordering = ['-search_count', 'suggestion_text']
    
    fieldsets = (
        ('Suggestion', {
            'fields': ('suggestion_text', 'normalized_text')
        }),
        ('Statistics', {
            'fields': ('search_count', 'result_count', 'click_through_rate')
        }),
        ('Categorization', {
            'fields': ('categories', 'locale_codes')
        }),
        ('Management', {
            'fields': ('is_active', 'is_promoted')
        }),
        ('Timestamps', {
