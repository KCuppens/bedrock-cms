...
class SearchIndex(models.Model):
    ...
    search_tags = models.JSONField(
        default=list,
        help_text="Tags for faceted search"
    )
    ...
    class Meta:
        ...
        indexes = [
            ...
            GinIndex(fields=['search_tags']) if GinIndex else models.Index(fields=['search_tags']),
        ]
        constraints = [
    ...
    def update_from_object(self, obj):
        ...
        if content_parts:
            ...
            self.excerpt = full_content[:200] + ('...' if len(full_content) > 200 else '')
        ...
        if config.locale_field and hasattr(obj, config.locale_field):
            ...
            if locale:
                self.locale_code = locale.code if hasattr(locale, 'code') else str(locale)
