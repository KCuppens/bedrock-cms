...
def search_collections(query: str, limit: int = 5) -> List[Dict[str, Any]]:
    ...
    return [{
        ...
        'type': 'collection',
        'icon': '📁',
        'description': collection.description[:100] if collection.description else '',
...
def search_translations(query: str, limit: int = 5) -> List[Dict[str, Any]]:
    ...
    return [{
        ...
        'type': 'translation',
        'icon': '🌐',
        'description': f"{translation.source_text[:50]}... → {translation.target_text[:50]}..." if translation.target_text else translation.source_text[:100],
...
def search_redirects(query: str, limit: int = 5) -> List[Dict[str, Any]]:
    ...
    redirects = Redirect.objects.filter(
        Q(from_path__icontains=query) |
        Q(to_path__icontains=query)
    )[:limit]
