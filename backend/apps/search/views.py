...
from .serializers import (
    ...
    SearchSuggestionSerializer, AutocompleteSerializer,
    SearchAnalyticsSerializer, SearchIndexSerializer,
    SearchQueryLogSerializer, BulkIndexSerializer
)
...
class SearchIndexListView(generics.ListAPIView):
    ...
    permission_classes = [IsAdminUser]
    filterset_fields = ['content_type', 'search_category', 'is_published', 'locale_code']
    ordering = ['-indexed_at']
...
def search_categories_view(request):
    ...
    """
    categories = SearchIndex.objects.values_list(
        'search_category', flat=True
    ).distinct().order_by('search_category')
