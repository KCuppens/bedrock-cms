...
class PerformanceReviewer:
    ...
    def analyze_api_performance(self):
        ...
        for method, endpoint, name in endpoints:
            ...
            if times:
                ...
                status_icon = "✅" if avg_time < 100 else "⚠️" if avg_time < 500 else "🔴"
                print(f"    {status_icon} Avg: {avg_time:.1f}ms, Range: {min_time:.1f}-{max_time:.1f}ms")
    ...
    def analyze_caching(self):
        ...
        print(f"    ✓ Set: {set_time:.2f}ms, Get: {get_time:.2f}ms, Delete: {delete_time:.2f}ms")
    ...
    def analyze_block_performance(self):
        ...
        for count in test_cases:
            ...
            for i in range(count):
                ...
                blocks.append({
                    ...
                    'props': {
                        ...
                        'content': f'Content for block {i}' * 10,  # Make it substantial
                    }
                })
            ...
            if times:
                ...
                status_icon = "✅" if avg_time < 200 else "⚠️" if avg_time < 1000 else "🔴"
                print(f"      {status_icon} {avg_time:.1f}ms total, {avg_time/count:.2f}ms per block")
    ...
    def generate_recommendations(self):
        ...
        slow_blocks = [count for count, data in block_results.items() 
    ...
    def output_results(self):
        ...
        if 'page_tree' in db:
            ...
            print(f"  • Naive: {pt['naive']['queries']} queries, {pt['naive']['time']*1000:.1f}ms")
            print(f"  • Optimized: {pt['optimized']['queries']} queries, {pt['optimized']['time']*1000:.1f}ms")
            print(f"  • Improvement: {pt['improvement']}")
        
        if 'blog_posts' in db:
            bp = db['blog_posts']
            print(f"Blog Post Queries:")
            print(f"  • Naive: {bp['naive']['queries']} queries, {bp['naive']['time']*1000:.1f}ms")
            print(f"  • Optimized: {bp['optimized']['queries']} queries, {bp['optimized']['time']*1000:.1f}ms")
        ...
        for name, data in self.results['api'].items():
            ...
            print(f"{status_icon} {name}: {data['avg_time_ms']}ms avg ({data['min_time_ms']}-{data['max_time_ms']}ms)")
        ...
        print(f"Operations: Set {ops['set_time_ms']}ms, Get {ops['get_time_ms']}ms, Delete {ops['delete_time_ms']}ms")
        ...
        for count, data in self.results['blocks'].items():
            status_icon = "✅" if data['status'] == 'good' else "⚠️" if data['status'] == 'warning' else "🔴"
            print(f"{status_icon} {count} blocks: {data['avg_time_ms']}ms total, {data['per_block_ms']}ms per block")
        ...
        print(f"Asset Queries: {ops['asset_query_ms']}ms → {ops['optimized_query_ms']}ms ({ops['improvement']} improvement)")
