...
def fix_model_files():
    ...
    for file_path in files:
        ...
        field_patterns = [
            ...
            (
                r"(\s+)(\w+)\s*=\s*models\.SlugField\(",
                r"\1\2: SlugField = models.SlugField(",
            ),
            (
                r"(\s+)(\w+)\s*=\s*models\.AutoField\(",
                r"\1\2: AutoField = models.AutoField(",
            ),
            (
                r"(\s+)(\w+)\s*=\s*models\.OneToOneField\(",
                r"\1\2: OneToOneField = models.OneToOneField(",
            ),
            (
                r"(\s+)(\w+)\s*=\s*models\.URLField\(",
                r"\1\2: URLField = models.URLField(",
            ),
            (
                r"(\s+)(\w+)\s*=\s*models\.GenericIPAddressField\(",
                r"\1\2: GenericIPAddressField = models.GenericIPAddressField(",
            ),
            (
                r"(\s+)(\w+)\s*=\s*models\.IntegerField\(",
                r"\1\2: IntegerField = models.IntegerField(",
            ),
        ]
        ...
        content = re.sub(r"(\w+):\s*(\w+):\s*(\w+)\s*=", r"\1: \2 =", content)
